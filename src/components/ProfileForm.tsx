...
export const ProfileForm = ({ profile, onSave, onCancel, isInline = false }: ProfileFormProps) => {
    ...
    return (
        <div className={`flex flex-col h-full ${isInline ? '' : 'p-4'}`}>
            <div className={`flex-1 overflow-y-auto space-y-4 ${isInline ? 'p-6' : ''}`}>
                {/* Remarks */}
                <div>
            ...
            {/* Footer */}
            <div className={`border-t border-border flex justify-end gap-2 ${isInline ? 'mt-4 pt-4' : 'p-4'}`}>
                {onCancel && (
                    <button
                        onClick={onCancel}
