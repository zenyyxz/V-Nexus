...
export const LogsView = () => {
    ...
    const { logs, clearLogs } = useApp() // consume global logs
    ...
    const parseLog = (log: string) => {
        ...
        // Fallback for old logs or simple strings
    ...
    const filteredLogs = logs.filter(log => {
        ...
        return low.includes(`[${filterLevel}]`)
    })

    // Auto-scroll logic
    useEffect(() => {
        if (autoRefresh && !userScrolled && logsEndRef.current) {
            logsEndRef.current.scrollIntoView({ behavior: 'smooth' })
        }
    }, [logs, userScrolled, autoRefresh])
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex bg-background/50 rounded-lg p-0.5 border border-border/50">
                                <button
                                    ...
                                    className={`px-2.5 py-0.5 text-[10px] font-medium rounded transition-colors ${filterLevel === 'all' ? 'bg-zinc-700 text-white shadow-sm' : 'text-secondary hover:text-primary'}`}
                                >
                                    All
                                </button>
                                <button
                                    onClick={() => setFilterLevel('warning')}
                                    className={`px-2.5 py-0.5 text-[10px] font-medium rounded transition-colors ${filterLevel === 'warning' ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/20 shadow-sm' : 'text-secondary hover:text-yellow-400'}`}
                                >
                                    Warnings
                                </button>
                                <button
                                    onClick={() => setFilterLevel('error')}
                                    className={`px-2.5 py-0.5 text-[10px] font-medium rounded transition-colors ${filterLevel === 'error' ? 'bg-red-500/20 text-red-400 border border-red-500/20 shadow-sm' : 'text-secondary hover:text-red-400'}`}
                                >
                                    Errors
                                </button>
                            </div>
                        </div>
                        <span className="text-xs text-secondary">{filteredLogs.length} {t('logs_entries')}</span>
                    </div>
                </div>
                    ...
                    {logs.length === 0 ? (
                        ...
                        <div className="space-y-0.5">
                            {filteredLogs.map((log, index) => {
                                const { timestamp, level, message } = parseLog(log)
                                return (
                                    <div key={index} className="flex items-start gap-3 hover:bg-zinc-800/30 px-2 py-1 rounded transition-colors break-words">
                                        {timestamp && (
                                            <span className="text-zinc-600 shrink-0 font-mono text-[11px] pt-0.5 select-none">{timestamp}</span>
                                        )}
                                        <span className={`shrink-0 text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-sm bg-opacity-10 w-16 text-center select-none ${level === 'error' ? 'bg-red-500 text-red-500' :
                                            level === 'warning' ? 'bg-yellow-500 text-yellow-500' :
                                                level === 'debug' ? 'bg-zinc-500 text-zinc-400' :
                                                    'bg-blue-500 text-blue-400'
                                            }`}>
                                            {level}
                                        </span>
                                        <span className={`${getLogColor(level)} flex-1`}>{message}</span>
                                    </div>
                                )
                            })}
                            <div ref={logsEndRef} />
