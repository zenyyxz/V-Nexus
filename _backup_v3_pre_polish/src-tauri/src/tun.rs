    ...
    *state.default_gateway.lock().unwrap() = Some(gateway.clone());
    ...
    if let Some(stderr) = child.stderr.take() {
        ...
        thread::spawn(move || {
            ...
            for line in reader.lines() {
                if let Ok(msg) = line {
                    emit_log(&app_handle_err, "ERROR", &format!("[Tun2Socks] {}", msg));
                }
            }
        });
    }
...
#[tauri::command]
pub async fn stop_tun(state: State<'_, TunProcess>) -> Result<String, String> {
    ...
    if let Some(ip) = ip_lock.take() {
        println!("[Tun] Cleaning up route for {}", ip);
    ...
    if let Some(gw) = gw_lock.take() {
        println!("[Tun] Restoring default gateway state: {}", gw);
    }
