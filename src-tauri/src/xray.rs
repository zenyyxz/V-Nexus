...
pub async fn start_xray(
    ...
    // Log path finding
    let _ = app.emit("xray-log", LogPayload {
        level: "INFO".into(),
        message: "Resolving Xray binary path...".into(),
    });
    ...
    let _ = app.emit("xray-log", LogPayload {
        level: "INFO".into(),
        message: format!("Xray binary found at: {:?}", xray_path),
    });

    if !xray_path.exists() {
        let msg = format!("CRITICAL: Xray binary NOT found at {:?}", xray_path);
        println!("{}", msg);
        let _ = app.emit("xray-log", LogPayload {
            level: "ERROR".into(),
            message: msg,
        });
        return Err("Xray binary missing".into());
    }
    ...
    let pid = child.id();
    let start_msg = format!("Xray process spawned successfully. PID: {}", pid);
    println!("{}", start_msg);
    let _ = app.emit("xray-log", LogPayload {
        level: "INFO".into(),
        message: start_msg,
    ...
    // Stream logs ... (Existing stream logic)
    let app_handle_out = app.clone();
    thread::spawn(move || {
        let reader = BufReader::new(stdout);
        for line in reader.lines() {
            if let Ok(msg) = line {
                println!("[Xray INFO] {}", msg);
...
#[tauri::command]
pub async fn stop_xray(app: AppHandle, state: State<'_, XrayProcess>) -> Result<String, String> {
    let mut child_lock = state.child.lock().map_err(|e| e.to_string())?;

    if let Some(mut child) = child_lock.take() {
        let pid = child.id();
        let msg = format!("Stopping Xray process (PID: {})...", pid);
        println!("{}", msg);
        let _ = app.emit("xray-log", LogPayload { level: "INFO".into(), message: msg });
        ...
        let _ = app.emit("xray-log", LogPayload { 
            level: "INFO".into(), 
            message: "Xray process terminated.".into() 
        });

        Ok("Xray stopped".into())
    } else {
        println!("Stop called but Xray not running.");
        ...
        if let Ok(json_output) = serde_json::from_str::<serde_json::Value>(&stdout) {
            if let Some(stats) = json_output.get("stat").and_then(|s| s.as_array()) {
                for stat in stats {
                    ...
                    // Filter: Only count OUTBOUND traffic that is NOT direct/block/api
                    // This creates a "Proxy Usage" stat, effectively.
                    if name.starts_with("outbound>>>") 
         ...
         let _ = app.emit("xray-log", LogPayload {
               level: "ERROR".into(),
               message: format!("Stats Command Failed: {}", stderr)
          });
    }
