# TUN Prototype Success Report

> **Verdict:** SUCCESS
> **Verified Apps:** WhatsApp, Speedtest, Antigravity (Non-Proxy App)
> **ISP Bypass:** Confirmed (Zoom Package Trick via `aka.ms` SNI)

## 1. How It Worked (The "Secret Sauce")

The failure of previous implementations was due to routing loops where the encrypted VPN traffic tried to enter the VPN tunnel itself. The prototype fixed this using **Split Routing with Metrics**.

### The Flow

1. **Apps (WhatsApp/Discord):** Try to connect to internet.
2. **OS Routing Table:** Sees `0.0.0.0/0` -> `10.0.0.1` (TUN Interface) has **Metric 1** (Highest Priority).
3. **TUN Hijack:** Traffic is forced into `Tun2Socks` -> `Xray`.
4. **Encryption:** Xray wraps the traffic in VLESS + TLS (SNI: `aka.ms`).
5. **Egress (The Fix):** Xray sends the *encrypted* packet to the VPS IP (`161.118.248.52`).
6. **Bypass Route:** Windows sees a **specific route** for `161.118...` pointing to your **Physical Gateway** (`192.168.x.x`) with **Metric 5**.
    * *Why this works:* Metric 5 is "worse" than Metric 1, BUT a **specific IP match** beats a **generic 0.0.0.0 match**. So connection to the VPS *escapes* the tunnel.
7. **ISP:** Sees traffic to `161.118...` that looks like `aka.ms` (Microsoft/Zoom). Allows it.

## 2. Running Instructions

**Prerequisites:**
* Run terminal as **Administrator**.
* Ensure `resources/xray` and `resources/tun2socks` exist.
* Ensure `singapore.json` is in `src-tauri/`.

**Command:**

```powershell
cd src-tauri
cargo run --bin tun_manual
```

**Usage:**

1. Wait for "SYSTEM-WIDE TUNNEL IS ACTIVE!".
2. Minimize terminal (Do NOT close).
3. Use internet.
4. Press **ENTER** in terminal to disconnect and clean up.

## 3. The Working Code (`tun_manual.rs`)

```rust
use std::env;
use std::io::{self, Write};
use std::process::{Command, Stdio};
use std::thread;
use std::time::Duration;

// CONSTANTS (V2 Logic)
const TUN_IP: &str = "10.0.0.2";
const TUN_GW: &str = "10.0.0.1";
const TUN_MASK: &str = "255.255.255.0";
const TUN_METRIC: &str = "1";
const BYPASS_METRIC: &str = "5";

fn main() {
    println!("=== V-Nexus MANUAL SYSTEM TEST ===");
    println!("This mode keeps the tunnel open for you to test apps like WhatsApp/Discord.");
    
    // Server Details
    let server_ip = "161.118.248.52"; 
    let socks_port = "10808";

    println!("Target Server: {}", server_ip);

    // 1. Terminate Existing
    println!("\n[1/9] Terminating existing processes...");
    let _ = Command::new("taskkill").args(&["/F", "/IM", "xray.exe"]).output();
    let _ = Command::new("taskkill").args(&["/F", "/IM", "tun2socks-windows-amd64.exe"]).output();
    thread::sleep(Duration::from_secs(1));

    // 2. Clean Routes
    println!("[2/9] Cleaning old routes...");
    let _ = run_cmd("route", &["delete", server_ip]);
    let _ = run_cmd("route", &["delete", "0.0.0.0", "mask", "0.0.0.0", TUN_GW]);
    let _ = run_cmd("route", &["delete", "0.0.0.0", "mask", "0.0.0.0", "10.4.2.1"]);

    // 3. Detect and Add Bypass Route
    println!("[3/9] Adding Physical Bypass Route...");
    let gateway = match get_physical_gateway() {
        Ok(gw) => gw,
        Err(e) => {
            eprintln!("CRITICAL ERROR: Failed to detect physical gateway: {}", e);
            return;
        }
    };
    println!("Detected Physical Gateway: {}", gateway);
    
    if let Err(e) = run_cmd("route", &["add", server_ip, "mask", "255.255.255.255", &gateway, "metric", BYPASS_METRIC]) {
        eprintln!("Failed to add bypass route: {:?}", e);
    }

    // 4. Start Xray (Singapore Config) - SEPARATE WINDOW
    println!("[4/9] Starting Xray Core (New Window)...");
    let xray_path = std::env::current_dir().unwrap().join("resources/xray/xray.exe");
    let config_path = std::env::current_dir().unwrap().join("singapore.json");
    
    if xray_path.exists() && config_path.exists() {
        let xray_exe = xray_path.to_str().unwrap();
        let config_str = config_path.to_str().unwrap();
        let ps_cmd = format!("& '{}' run -c '{}'", xray_exe, config_str);

        let _ = Command::new("cmd")
            .args(&[
                "/C", "start", "powershell", 
                "-NoExit", 
                "-Command", &ps_cmd
            ])
            .spawn()
            .expect("Failed to spawn Xray");
        thread::sleep(Duration::from_secs(3));
    } else {
        eprintln!("CRITICAL: Xray binary or singapore.json not found!");
        return;
    }

    // 5. Start Tun2Socks - SEPARATE WINDOW
    println!("[5/9] Starting Tun2Socks (New Window)...");
    let tun2socks_path = std::env::current_dir().unwrap().join("resources/tun2socks/tun2socks-windows-amd64.exe");
    
    if !tun2socks_path.exists() {
        eprintln!("CRITICAL: Tun2Socks binary not found at {:?}", tun2socks_path);
        return;
    }

    let exe_path = tun2socks_path.to_str().unwrap();
    let ps_command = format!(
        "& '{}' -device tun://tun0 -proxy socks5://127.0.0.1:{} -loglevel info",
        exe_path, socks_port
    );

    let _ = Command::new("cmd")
        .args(&[
            "/C", 
            "start", 
            "powershell",
            "-NoExit",
            "-Command", 
            &ps_command
        ])
        .spawn();

    println!("Tun2Socks launching in PowerShell window...");
    thread::sleep(Duration::from_secs(5));

    // 6. Configure IP (Netsh)
    println!("[6/9] Configuring TUN Interface IP & Gateway...");
    let ip_cmd = format!(
        "netsh interface ip set address name=\"tun0\" static {} {} gateway={} gwmetric={}",
        TUN_IP, TUN_MASK, TUN_GW, TUN_METRIC
    );
    if let Err(e) = run_cmd("cmd", &["/C", &ip_cmd]) {
        eprintln!("Failed to set IP: {:?}", e);
    }

    // 7. Configure DNS
    println!("[7/9] Configuring DNS...");
    let ps_dns = format!(
        "Set-DnsClientServerAddress -InterfaceAlias 'tun0' -ServerAddresses ('1.1.1.1', '1.0.0.1')"
    );
    if let Err(e) = run_cmd("powershell", &["-ExecutionPolicy", "Bypass", "-Command", &ps_dns]) {
        eprintln!("Failed to set DNS: {:?}", e);
    }

    // 8. Interactive Wait
    println!("\n===========================================");
    println!("      SYSTEM-WIDE TUNNEL IS ACTIVE!");
    println!("===========================================");
    println!("All traffic is now routed through the Singapore server.");
    println!("Please open WhatsApp, Discord, or Browser to verify.");
    println!("Keep this window OPEN.");
    println!("===========================================");
    println!("Press ENTER only when you are done testing to CLEAN UP.");
    
    let mut input = String::new();
    let _ = io::stdin().read_line(&mut input);

    // CLEANUP
    println!("Stopping...");
    let _ = Command::new("taskkill").args(&["/F", "/IM", "xray.exe"]).output();
    let _ = Command::new("taskkill").args(&["/F", "/IM", "tun2socks-windows-amd64.exe"]).output();
    
    println!("Cleaning routes...");
    let _ = run_cmd("route", &["delete", server_ip]);
    let _ = run_cmd("route", &["delete", "0.0.0.0", "mask", "0.0.0.0", TUN_GW]);

    println!("Cleanup Complete.");
}

// Helpers
fn run_cmd(cmd: &str, args: &[&str]) -> Result<(), String> {
    println!("> {} {:?}", cmd, args);
    let status = Command::new(cmd)
        .args(args)
        .status()
        .map_err(|e| e.to_string())?;
    
    if !status.success() {
        return Err(format!("Exited with code {:?}", status.code()));
    }
    Ok(())
}

fn get_physical_gateway() -> Result<String, String> {
    let ps_cmd = "Get-NetRoute -DestinationPrefix \"0.0.0.0/0\" -AddressFamily IPv4 | \
                  Where-Object { \
                    $_.NextHop -ne \"0.0.0.0\" -and \
                    $_.InterfaceAlias -notmatch \"tun\" -and \
                    $_.InterfaceAlias -notmatch \"TAP\" -and \
                    $_.InterfaceAlias -notmatch \"V-Nexus\" \
                  } | \
                  Sort-Object -Property RouteMetric | \
                  Select-Object -First 1 -ExpandProperty NextHop";

    let output = Command::new("powershell")
        .args(&["-NoProfile", "-Command", ps_cmd])
        .output()
        .map_err(|e| e.to_string())?;

    if !output.status.success() {
        return Err(String::from_utf8_lossy(&output.stderr).to_string());
    }

    let gw = String::from_utf8_lossy(&output.stdout).trim().to_string();
    if gw.is_empty() || !gw.contains('.') {
        return Err("No valid gateway found".into());
    }
    Ok(gw)
}
```
